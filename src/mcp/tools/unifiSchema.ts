...
export async function handler(input: { tag?: string; path?: string }) {
  ...
  if (path) {
    ...
    if (!specPathEntry) {
      return ResponseFormatter.error(
        "Path not found",
        `'${path}' not found. Available: ${Object.keys(spec.paths).slice(0, 10).join(", ")}...`
      );
    }
  ...
  if (tag) {
    const matchingPaths = Object.entries(spec.paths)
      .filter(([, ops]) => {
        const tagList = (ops as any).tags ?? [];
        return tagList.includes(tag);
      })
      .map(([p]) => p);

    if (matchingPaths.length === 0) {
      const allTags = [...new Set(Object.values(spec.paths).flatMap((ops: any) => ops.tags ?? []))];
      return ResponseFormatter.error(
        "Tag not found",
        `Tag '${tag}' not found. Available: ${allTags.join(", ")}`
      );
    }

    return ResponseFormatter.success(
      `Tag: ${tag} (${matchingPaths.length} paths)`,
      { apiType, tag, paths: matchingPaths }
    );
  }
  ...
  for (const [p, ops] of Object.entries(spec.paths)) {
    const tags = (ops as any).tags ?? ["untagged"];
    for (const t of tags) {
  ...
  return ResponseFormatter.success(
    `UniFi Network API — ${Object.keys(spec.paths).length} paths across ${tagMap.size} tags`,
    {
      apiType,
      apiTypeDescription: getApiTypeDescription(apiType),
      baseUrl: apiType === "local" ? "https://192.168.100.1" : "https://api.ui.com",
