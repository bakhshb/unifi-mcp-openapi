...
export const schema = {
  path: z
    ...
    .min(1)
    .describe(
      'The API path as defined in the OpenAPI spec, e.g. "/v1/sites/{siteId}/devices". ' +
  ...
  pathParams: z
    ...
    .optional()
    .describe(
      'Path parameter values to substitute into the URL, e.g. { "siteId": "abc123", "deviceId": "xyz" }'
    ),
  params: z
    .record(z.unknown())
    .optional()
...
export const description =
  "Execute any UniFi Network Cloud API operation. Specify the path from the OpenAPI spec " +
...
export async function handler(input: {
  ...
  if (!spec.paths[path]) {
    return ResponseFormatter.error(
      ...
      `"${path}" is not a valid path. Use unifi-api-schema to discover available paths.`
    );
  }
  ...
  try {
    ...
    if (error instanceof AxiosError && error.response) {
      ...
      if (status === 401) {
        return ResponseFormatter.error(
          ...
          "Check UNIFI_API_KEY — ensure it is a valid UniFi Cloud API key"
        );
      }
      if (status === 403) {
        return ResponseFormatter.error(
          "Access denied",
          `Insufficient permissions for ${url}. Ensure the API key has the required scopes.`
        );
      }
      if (status === 404) {
        return ResponseFormatter.error(
      ...
      if (status === 429) {
        return ResponseFormatter.error(
          "Rate limited",
          "Too many requests. The UniFi Cloud API has rate limits — wait before retrying."
        );
      }
      if (status >= 500) {
        return ResponseFormatter.error(
          "UniFi Cloud API server error",
          `HTTP ${status} while processing ${url}`
        );
      }
    ...
    if (error instanceof AxiosError && error.code === "ECONNABORTED") {
      return ResponseFormatter.error(
        "Request timeout",
        `Request to ${url} timed out. Increase UNIFI_TIMEOUT if needed.`
      );
    }
